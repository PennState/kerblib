...
void usage()
{
  std::cout << "Usage:\nadminLock -a adminPrincipal -k keytab -p principal -r kerbRealm [-m logMessage]" << std::endl;
}
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    if (!logMessage.empty())
      session.lockUser(principal, logMessage);
    else
      session.lockUser(principal);
  }
  ...
  {
    std::cout << "Failed initialization: " << utie.what() << std::endl;
  }
  catch(...)
  {
